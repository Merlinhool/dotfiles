...
if [[ "$TERM" != "dumb" ]] && [[ "$DISABLE_LS_COLORS" != "true" ]]; then
  ...
  PROMPT='${SSH_CONNECTION:+"[%F{9}%n%f%F{7}@%f%F{3}%m%f]"} %{%F{45}%}$(prompt_pwd)$(git_prompt_info)'$'\n''${lreturn_status}%{$reset_color%} '
  ...
  RPROMPT='$(vi_mode_prompt_info)${return_status}$(git_prompt_status)%{$reset_color%}'
